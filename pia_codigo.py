...
if file_exists:
  print(f'{"*" * 25} ARCHIVO DB ENCONTRADO {"*" * 24}')
else:
  print(f'{"*" * 9} ARCHIVO DB NO ENCONTRADO, PRIMERA EJECUCIÓN ASUMIDA {"*" * 8}')
  try:
...
while True:
  print(f'\n{"-" * 70}')
  print(f'{" " * 23}    MENÚ DE OPCIONES    {" " * 23}')
  print(f'{"-" * 70}')
  print(f"\n [1] Submenú - Reservaciones\n",
        "[2] Submenú - Reportes\n",
  ...
  if opcion_menu == 1:
    while True:
      print(f'\n{"-" * 70}')
      print(f'{" " * 22} SUBMENÚ DE RESERVACIONES {" " * 22}')
      print(f'{"-" * 70}')
      ...
      while True:
        try:
          opcion_reservaciones = int(input("SELECCIONE UNA OPCIÓN: "))
        except ValueError:
          print("INGRESE UN VALOR NUMÉRICO\n")
        except Exception:
          print(f"OCURRIÓ UN PROBLEMA {sys.exc_info()[0]}\n")
      ...
      if opcion_reservaciones == 1:
        ...
        if registros_sala and registros_cliente:
          print(f'\n{"_" * 70}')
          print(f'{" " * 21} REGISTRO DE UNA RESERVACIÓN {" " * 20}')
          print(f'{"_" * 70}')
          ...
          while True:
            try:
              ...
              if fecha_actual <= fecha_valida:
                ...
                print("LA RESERVACIÓN DEBE DE HACERSE CON 2 DIAS DE ANTICIPACIÓN ANTES DEL EVENTO\n")

          try:
            with sqlite3.connect("registros.db", detect_types = sqlite3.PARSE_DECLTYPES | sqlite3.PARSE_COLNAMES) as conn:
              mi_cursor = conn.cursor()
              valores = {"fecha_reservacion":fecha_datetime, "clave_sala":clave_sala}
              mi_cursor.execute("SELECT clave_turno FROM reservacion WHERE (fecha = :fecha_reservacion) AND (clave_sala = :clave_sala);", valores)
              registros = mi_cursor.fetchall()

              if registros:
                turnos_posibles = set((1,2,3))
                turnos_escogidos = set([numero for (numero,) in registros])
                turnos_disponibles = sorted(list(turnos_posibles - turnos_escogidos))
              else:
          ...
          if turnos_disponibles:
            ...
            if 1 in turnos_disponibles:
              print(f"\n1) MATUTINO")
            if 2 in turnos_disponibles:
              print("2) VESPERTINO")
            if 3 in turnos_disponibles:
              print("3) NOCTURNO\n")

            while True:
              try:
                clave_turno = int(input("SELECCIONE EL TURNO DE LA RESERVACIÓN: "))
              except ValueError:
                print("INGRESE UN VALOR NUMÉRICO\n")
              except Exception:
                print(f"OCURRIÓ UN PROBLEMA {sys.exc_info()[0]}\n")
              else:
                if clave_turno in turnos_disponibles:
                  break
                else:
            ...
            while True:
              try:
                ...
                elif nombre_reservacion.isspace():
                  print("EL NOMBRE DEL EVENTO NO PUEDE CONSTAR DE PUROS ESPACIOS EN BLANCO\n")
                else:
                  break
            ...
            print(f'\n{"*" * 18} RESERVACIÓN REGISTRADA CON ÉXITO {"*" * 18}')
          else:
            print("\nYA SE ENCUENTRAN TODOS LOS TURNOS RESERVADOS PARA LA FECHA Y SALA SELECCIONADA")

        else:
          print("\nNO SE CUENTA CON CLIENTES Y/O SALAS REGISTRADAS")
        ...
        try:
          with sqlite3.connect("registros.db") as conn:
            ...
            registros_reservacion = mi_cursor.fetchall()
        except Error as e:
          print (e)
        except Exception:
          print(f"OCURRIÓ UN PROBLEMA {sys.exc_info()[0]}\n")
        ...
        if registros_reservacion:
          print(f'\n{"_" * 70}')
          print(f'{" " * 22} EDITAR NOMBRE DEL EVENTO {" " * 22}\n')
          print(f'{"_" * 70}')
          ...
          while True:
            try:
              clave = int(input("INGRESE LA CLAVE DE LA RESERVACIÓN A MODIFICAR: "))
            except ValueError:
              print("INGRESE UN VALOR NUMÉRICO\n")
            except Exception:
              print(f"OCURRIÓ UN PROBLEMA {sys.exc_info()[0]}\n")
              ...
              if clave in claves_reservacion:
                ...
                print("INGRESE UNA CLAVE DE RESERVACIÓN EXISTENTE\n")
          ...
          print(f'\n{"*" * 24} MODIFICACIÓN EXITOSA {"*" * 24}')
        else:
          print("\nNO SE CUENTA CON RESERVACIONES REGISTRADAS")
        ...
        if registros_sala:
          print(f'\n{"_" * 70}')
          print(f'{" " * 18} CONSULTAR DISPONIBILIDAD DE SALAS {" " * 17}')
          print(f'{"_" * 70}')
          ...
          try:
            with sqlite3.connect("registros.db") as conn:
              ...
              if registros_reservacion:
                ...
                for clave_sala, nombre_sala, nombre_turno in registros_posibles:
                  print("{:<6} {:<10} {:<20}".format(clave_sala, nombre_sala, nombre_turno))
            print("*" * 29)
          except Error as e:
            print (e)
          ...
          print("\nNO SE CUENTA CON SALAS REGISTRADAS")
        ...
        if registros_reservacion:
          print(f'\n{"_" * 70}')
          print(f'{" " * 21} ELIMINACIÓN DE RESERVACIÓN {" " * 21}\n')
          print(f'{"_" * 70}')

          print(f'\n{"*" * 9} EVENTOS EXISTENTES {"*" * 9}')
          print("*" * 38)
          print("{:<6} {:<20} {:<10}".format('CLAVE','NOMBRE','FECHA'))
          print("*" * 38)

          try:
            with sqlite3.connect("registros.db", detect_types = sqlite3.PARSE_DECLTYPES | sqlite3.PARSE_COLNAMES) as conn:
              mi_cursor = conn.cursor()
              mi_cursor.execute("SELECT clave_reservacion, nombre, fecha FROM reservacion;")
              registros = mi_cursor.fetchall()

              if registros:
                for clave_reservacion, nombre, fecha in registros:
                  fecha = fecha.date().strftime('%d/%m/%Y')
                  print("{:<6} {:<20} {:<10}".format(clave_reservacion, nombre, fecha))
                print(f'{"*" * 38}\n')
          except Error as e:
            print (e)
          except Exception:
          ...
          claves_reservacion = [clave_reservacion for clave_reservacion, nombre, fecha in registros]
          ...
          if fecha_actual <= fecha_valida:
            ...
            while True:
              try:
                eleccion = input("¿CONFIRMAR ELIMINACIÓN?, ESTE ES UN PROCEDIMIENTO QUE NO PUEDE SER DESHECHO [S/N]: ")
              except Exception:
                print(f"OCURRIÓ UN PROBLEMA {sys.exc_info()[0]}\n")
              else:
            ...
            if eleccion.upper() == "S":
              ...
              print(f'\n{"*" * 24} ELIMINACIÓN NO HECHA {"*" * 24}')
          else:
            print("SOLAMENTE SE PUEDEN ELIMINAR RESERVACIONES CON TRES DIAS DE ANTICIPACIÓN\n")
        else:
          print("\nNO SE CUENTA CON RESERVACIONES REGISTRADAS")
    ...
    while True:
      print(f'\n{"-" * 70}')
      print(f'{" " * 23}    SUBMENÚ DE REPORTES    {" " * 23}')
      print(f'{"-" * 70}')
      ...
      while True:
        try:
          opcion_reportes = int(input("SELECCIONE UNA OPCIÓN: "))
        except ValueError:
          print("INGRESE UN VALOR NUMÉRICO\n")
        except Exception:
          print(f"OCURRIÓ UN PROBLEMA {sys.exc_info()[0]}\n")
      ...
      if opcion_reportes == 1:
        ...
        if registros_reservacion:
          print(f'\n{"_" * 70}')
          print(f'{" " * 22} REPORTE DE RESERVACIONES {" " * 22}')
          print(f'{"_" * 70}')
        ...
        if registros_reservacion:
          print(f'\n{"_" * 70}')
          print(f'{" " * 18} REPORTE DE RESERVACIONES EN EXCEL {" " * 17}')
          print(f'{"_" * 70}')
          ...
          for nombre_sala, nombre_cliente, nombre_reservacion, nombre_turno in registros:
            ...
            hoja.cell(row = renglon, column = 1).alignment = Alignment (horizontal='center')
            hoja.cell(row = renglon, column = 3).alignment = Alignment (horizontal='center')
            hoja.cell(row = renglon, column = 5).alignment = Alignment (horizontal='center')
            hoja.cell(row = renglon, column = 7).alignment = Alignment (horizontal='center')

            renglon += 1

          nombre_archivo = f"reporte_excel_{fecha_modificada.day}-{fecha_modificada.month}-{fecha_modificada.year}.xlsx"
          libro.save(nombre_archivo)

          print(f'\n{"*" * 27} REPORTE CREADO {"*" * 27}')
        else:
          print("\nNO SE CUENTA CON RESERVACIONES REGISTRADAS")
    ...
    print(f'\n{"_" * 70}')
    print(f'{" " * 23}  REGISTRO DE UNA SALA  {" " * 23}')
    print(f'{"_" * 70}')
    ...
    while True:
      try:
        ...
        if nombre_sala == "":
          ...
          print("EL NOMBRE DE LA SALA NO PUEDE CONSTAR DE PUROS ESPACIOS EN BLANCO\n")
        else:
          break
    ...
    print(f'\n{"*" * 22} SALA REGISTRADA CON ÉXITO {"*" * 21}')

  elif opcion_menu == 4:
    print(f'\n{"_" * 70}')
    print(f'{" " * 20} REGISTRO DE UN NUEVO CLIENTE {" " * 20}')
    print(f'{"_" * 70}')
    ...
    while True:
      try:
        ...
        if nombre_cliente == "":
          ...
          print("EL NOMBRE DEL CLIENTE NO PUEDE CONSTAR DE PUROS ESPACIOS EN BLANCO\n")
        else:
          break
    ...
    print(f'\n{"*" * 20} CLIENTE REGISTRADO CON ÉXITO {"*" * 20}')
